...
int main()
{
	...
	if (!my_jack_manager.create_client("hertz")) { cout << "Cannot find Jack Audio Server - Exiting." << endl; return false; }
	...
	//------------ INITIALISE DEFAULT SONG
	
	song default_song;;
	default_song.set_name("Default Song");
	default_song.load_midi_file("test5.mid");
	...
	default_song.screen_width = main_gui.screen_width;
	default_song.screen_height = main_gui.screen_height;
	
	int panel_mixer = main_gui.create_widget(widget_type_panel, win, 1,0.5, 42,38, "MIXER");
	...
	for (int trk=0; trk < default_song.get_number_of_tracks(); trk++)
	{
		float track_x_pos = 0.5 + (trk * 3.25);
		int wid = main_gui.create_widget(widget_type_grid, win, track_x_pos, 15, 3.2, 22, "");
		main_gui.set_widget_parent(win, wid, panel_mixer);
		
		default_song.tracks[trk].x = main_gui.windows[win].widgets[wid]->x;
		default_song.tracks[trk].y = main_gui.windows[win].widgets[wid]->y;
		default_song.tracks[trk].w = main_gui.windows[win].widgets[wid]->w;
		default_song.tracks[trk].h = main_gui.windows[win].widgets[wid]->h;
	...
	{	
		...
		ss.str("");
		ss << trk;
		int track_title_button = main_gui.create_widget(widget_type_button, win, track_x_pos, 1, 3, 1, default_song.get_track_name(trk));
		...
		int pan_widget_number = main_gui.create_widget(widget_type_knob, win, track_x_pos - 0.25, 9, 2,3, "PAN");
		...
		int bunghole = main_gui.create_widget(widget_type_switch, win, track_x_pos - 0.25, 12, 2,3, "GRONK");
		main_gui.set_widget_parent(win, bunghole, panel_mixer);
	}
	...
	for (int trk=0; trk<default_song.get_number_of_tracks(); trk++)
	{
		default_song.draw_track_display(vg, trk);
	...
	default_song.find_visible_blocks();
	...
	do
	{
		...
		if (main_gui.windows[current_window].draw_all)  // AN EVENT REQUIRING ALL OF THE WINDOW TO BE REDRAW HAS OCCURRED
		{
			...
			for (int trk=0; trk<default_song.get_number_of_tracks(); trk++)
			{			
				default_song.tracks[trk].x = main_gui.windows[current_window].widgets[widget_grid_edit[trk]]->x;
				default_song.tracks[trk].y = main_gui.windows[current_window].widgets[widget_grid_edit[trk]]->y;
				default_song.tracks[trk].w = main_gui.windows[current_window].widgets[widget_grid_edit[trk]]->w;
				default_song.tracks[trk].h = main_gui.windows[current_window].widgets[widget_grid_edit[trk]]->h;
				default_song.draw_track_display(vg, trk);
				glfwSwapBuffers(main_gui.windows[current_window].window);
			}
			nvgEndFrame(vg);
			...
			if (current_widget > -1)
			{
				if (main_gui.windows[current_window].widgets[current_widget]->type == widget_type_grid)
				{
					current_song_track = song_track_widget_number[current_widget];
						default_song.draw_track_display(vg, current_song_track);
						nvgEndFrame(vg);
						glfwSwapBuffers(main_gui.windows[current_window].window);
				}
			}
